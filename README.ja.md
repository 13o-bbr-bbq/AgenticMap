...
> AgenticMap は **Amazon Bedrock AgentCore** を対象とした OSS の AI セキュリティポスチャー管理（AI-SPM）ツールです。AgentCore の設定監査（静的）とレッドチーミング（動的）を組み合わせ、従来の静的解析ツールでは検出不能なリスクを検出します。また、日本国内の AI 関連規制に準拠しています。

AgenticMap は AIエージェントシステム（単体のAIエージェント、マルチエージェントシステム）に固有の脆弱性および設定不備にフォーカスし、従来の CSPM やAIレッドチーミングツールでは捉えられない領域を対象とします。

> ステータス: 現在対応しているAIエージェント基盤は Amazon Bedrock AgentCore です。Azure AI Foundry や Vertex AI への対応は順次行っていきます。

## ポジショニング

- **OSS** — 本ツール開発時点（2026年5月）の AI-SPM 市場は商用 SaaS（Microsoft Defender、Zenity、Wiz、Prisma Cloud、Orca、CrowdStrike、Zscaler）が主流。AgenticMap は OSS を維持し、監査可能性・エアギャップ運用・研究者による拡張性を確保します。
...
| 対象機能 | 設定不備の例（boto3 で取得するフィールド名） |
|---|---|
| **AgentCore Runtime** | `authorizerConfiguration.customJWTAuthorizer` 未設定（inbound 認証なし）、`networkConfiguration.networkMode=PUBLIC`（VPC 隔離なし） |
| **AgentCore Memory** | `eventExpiryDuration` 未設定（無期限保持）、`encryptionKeyArn` に CMK 未指定（PII マスキングは Memory ではなく Guardrail 側で扱う） |
| **AgentCore Gateway** | `authorizerType=NONE`（anonymous）、Tool target 過剰露出、`exceptionLevel=DEBUG`（情報漏洩） |
| **AgentCore Identity** | Workload Identity の `allowedResourceOauth2ReturnUrls` に wildcard / 組織外ドメインを許可（open redirect 経路） |
| **AgentCore Policy** | Gateway に `policyEngineConfiguration` 未設定、`mode=LOG_ONLY` のまま本番運用、Cedar に条件なしの `permit` ルール |
| **AgentCore Browser** | `networkConfiguration.networkMode=PUBLIC`（VPC 隔離なし）→ 任意 URL 経由で間接 PI 経路、`recording.enabled=false`（事後追跡不能） |
| **AgentCore Code Interpreter** | `networkConfiguration.networkMode=PUBLIC`（SANDBOX / VPC 未使用） |
| **AgentCore Observability** | CloudTrail データイベント OFF、ログが平文（`kmsKeyId` 未設定）、Application Signals / X-Ray 未配線 |
| **Bedrock Agents: Guardrail** | Agent に Guardrail 未アタッチ、`PROMPT_ATTACK` フィルタ / `piiEntities` 無効、組織レベル Enforced Guardrail Configuration 未設定 |
...
| **Bedrock Agents: Collaborator（マルチエージェント）** | サブエージェントに Guardrail 未適用、親エージェントより弱い authorizer |
